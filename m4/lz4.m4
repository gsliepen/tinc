...
AC_DEFUN([tinc_LZ4], [
  ...
  AS_IF([test "$enable_lz4" != 'no' -a "$enable_lz4_builtin" != 'no' ], [
    AC_MSG_CHECKING([for lz4 builtin])
    AS_IF([test -e ./src/lz4/lib/lz4.h], [
      AC_MSG_RESULT([submodule found])
      ...
      AC_MSG_ERROR([
*** Tinc bundled src/lz4/lib/lz4.h not found.
*** Use the --disable-lz4-builtin switch or run:
*** # git submodule init
