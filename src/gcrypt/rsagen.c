...
bool rsa_write_pem_public_key(rsa_t *rsa, FILE *fp) {
	...
	if(!ber_write_mpi(&derp1, &derlen1, &rsa->n)
		...
		logger(DEBUG_ALWAYS, LOG_ERR, _("Error while encoding RSA public key"));
		return false;
	}

	if(!pem_encode(fp, "RSA PUBLIC KEY", derbuf2, derlen2)) {
		logger(DEBUG_ALWAYS, LOG_ERR, _("Unable to write RSA public key: %s"), strerror(errno));
		return false;
	}
...
bool rsa_write_pem_private_key(rsa_t *rsa, FILE *fp) {
	...
	if(!ber_write_mpi(&derp1, &derlen1, &bits)
		...
		logger(DEBUG_ALWAYS, LOG_ERR, _("Error while encoding RSA private key"));
	}
...
if(!pem_encode(fp, "RSA PRIVATE KEY", derbuf2, derlen2)) {
	logger(DEBUG_ALWAYS, LOG_ERR, _("Unable to write RSA private key: %s"), strerror(errno));
	return false;
}
