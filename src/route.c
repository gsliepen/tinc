...
static bool checklength(node_t *source, vpn_packet_t *packet, length_t length) {
	if(packet->len < length) {
		logger(DEBUG_TRAFFIC, LOG_WARNING, _("Got too short packet from %s (%s)"), source->name, source->hostname);
		return false;
	} else {
		return true;
	}
...
static void clamp_mss(const node_t *source, const node_t *via, vpn_packet_t *packet) {
	...
	for(int i = 0; i < len;) {
		...
		logger(DEBUG_TRAFFIC, LOG_INFO, _("Clamping MSS of packet from %s to %s to %d"), source->name, via->name, newmss);
		...
		if(s->expires && s->expires < now.tv_sec) {
			if(debug_level >= DEBUG_TRAFFIC) {
				...
				if(net2str(netstr, sizeof(netstr), s)) {
					logger(DEBUG_TRAFFIC, LOG_INFO, _("Subnet %s expired"), netstr);
				}
			}
...
static void learn_mac(mac_t *address) {
	...
	if(!subnet) {
		logger(DEBUG_TRAFFIC, LOG_INFO, _("Learned new MAC address %x:%x:%x:%x:%x:%x"),
...
static void fragment_ipv4_packet(node_t *dest, vpn_packet_t *packet, length_t ether_size) {
	...
	if(ether_size + ip_size + todo != packet->len) {
		logger(DEBUG_TRAFFIC, LOG_WARNING, _("Length of packet (%d) doesn't match length in IPv4 header (%zu)"), packet->len, ether_size + ip_size + todo);
		return;
	}

	logger(DEBUG_TRAFFIC, LOG_INFO, _("Fragmenting packet of %d bytes to %s (%s)"), packet->len, dest->name, dest->hostname);
...
static void route_ipv4(node_t *source, vpn_packet_t *packet) {
	...
	if(!subnet) {
		logger(DEBUG_TRAFFIC, LOG_WARNING, _("Cannot route packet from %s (%s): unknown IPv4 destination address %d.%d.%d.%d"),
	...
	if(subnet->owner == source) {
		logger(DEBUG_TRAFFIC, LOG_WARNING, _("Packet looping back to %s (%s)!"), source->name, source->hostname);
		return;
	}
	...
	if(via == source) {
		logger(DEBUG_TRAFFIC, LOG_ERR, _("Routing loop for packet from %s (%s)!"), source->name, source->hostname);
		return;
	}
	...
	if(via && packet->len > MAX(via->mtu, 590) && via != myself) {
		logger(DEBUG_TRAFFIC, LOG_INFO, _("Packet for %s (%s) length %d larger than MTU %d"), subnet->owner->name, subnet->owner->hostname, packet->len, via->mtu);
...
static void route_ipv6(node_t *source, vpn_packet_t *packet) {
	...
	if(!subnet) {
		logger(DEBUG_TRAFFIC, LOG_WARNING, _("Cannot route packet from %s (%s): unknown IPv6 destination address %hx:%hx:%hx:%hx:%hx:%hx:%hx:%hx"),
		       source->name, source->hostname,
		       ntohs(dest.x[0]),
		       ntohs(dest.x[1]),
		       ntohs(dest.x[2]),
...
static void route_neighborsol(node_t *source, vpn_packet_t *packet) {
	...
	if(source != myself) {
		logger(DEBUG_TRAFFIC, LOG_WARNING, _("Got neighbor solicitation request from %s (%s) while in router mode!"), source->name, source->hostname);
		return;
	}
...
static void route_arp(node_t *source, vpn_packet_t *packet) {
	...
	if(source != myself) {
		logger(DEBUG_TRAFFIC, LOG_WARNING, _("Got ARP request from %s (%s) while in router mode!"), source->name, source->hostname);
		return;
	}
...
void route(node_t *source, vpn_packet_t *packet) {
	...
	case RMODE_ROUTER:
		...
		default:
			logger(DEBUG_TRAFFIC, LOG_WARNING, _("Cannot route packet from %s (%s): unknown type %hx"), source->name, source->hostname, type);
			break;
		}
